...
3. 借鉴 = 机制、结构、参数取值、**测试用例清单**（上游 `summer-checkin/tests/` 与 `interview-guide/frontend/src/**/*.test.ts` 是现成的行为规格）；确需搬代码时直接改造复用。上游三仓均为本人自有项目，**搬运不受许可约束、无需保留来源声明**；**成片（跨文件）搬运才需要先征得用户同意**。
