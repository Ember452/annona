...
| P0-10 Docker 交付 | `🅖 {app,frontend}/Dockerfile`, `🅢 Dockerfile`, `🅢 docker-compose.yml`, `🅖 docker-compose.yml`, `🅜 MockPilot-mian/docker-compose.yml` | 多阶段 build + healthcheck + depends_on 结构；MinIO 与 pgvector 镜像 pin 策略 | 三阶段（web-build 前置到同 Dockerfile）；服务命名 role 化（db/cache/storage/server/web）；MinIO 走 `MINIO_DEFAULT_BUCKETS` env 不建 init container（2026-09-26 补：镜像选型已改 PGSTY Silo，该 env 在 fork 上的生效性属待验证假设，见 [specs/2026-09-26-s3-storage-silo-adr.md](./specs/2026-09-26-s3-storage-silo-adr.md)）；nginx 不 mount static（走 server 内嵌 static）；pgvector 镜像用 `pgvector/pgvector:pg16` 不用 `postgres:16` |
...
## P1c 训练决策层（10 人日）——**可解释面试决策的载体**

**目标**：面试不再是随机出卷，且系统能解释自己为什么这么出。

| ID | 任务 | 验收 | 人日 | 依赖 |
|---|---|---|---|---|
| P1c-01 | `shared/signal`：`LearningSignalReader` 实现，**按 direction_id 聚合**信号——面试侧（得分、间隔天数）为主输入，学习侧（时长带质量、完成率）为相交增强；开工时扩展 SPI `SignalSnapshot` 方向维度（direction ADR 修订 3 遗留义务） | 同一用户同一天同方向的信号快照可复现（golden 测试）；方向无学习记录时快照信号为空且决策链正常 | 1.5 | P1a-04,P1b-07 |
...
| P1c-04 | `planner/guard`：样本量 <3 不调难度、某方向信号全 SELF_REPORTED 则该方向学习信号不参与（决策仍基于面试侧）、方向无学习记录为正常形态非降级、单方向 ≤40%、换模型后前 3 场只采基线 | 五种场景各有测试，且原因被写进输出 | 1.5 | P1c-03 |
...
| P1c-06 | 可解释面板（前端 `explain-panel`）：把 `reasons` 翻成人话，显示数据质量等级与"数据不足"状态；方向无学习记录时显示"决策基于面试表现"而非留白 | 三条真实决策可被非项目成员读懂；SELF_REPORTED 与"无学习记录"两种状态明确标注 | 1.5 | P1c-05 |
