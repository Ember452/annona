...
| P0 骨架与门禁 | 🔶 **评审后整改已推，等 CI 复跑** —— B1–B5 + 12 条整改 commit + D22/D23 修复已入主干；compose-smoke 首次真跑已绿（= 出口②主体证据到手），docker-it 因 D23 已红一次并修复 | `mvn -B -q verify` 本机绿 + compose-smoke 首次绿日志 + **待 docker-it 全绿日志**（回填阶段总结 §3 出口②） |
