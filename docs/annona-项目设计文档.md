...
| 一句话 | 自习室 + AI 模拟面试的双功能训练平台：两个功能平级独立、共享方向字典与基础设施 |
| 定位 | 不是「自习工具 + 面试工具 + 知识库工具」的简单拼盘——共用一套地基（身份、方向字典、检索、模型网关）；学习数据在方向相交时增强面试决策（见 §6），不相交时两线互不干扰 |
...
**年轮把第一类的输出接到第二类的输入上——但作为增强，而不是前提。** 面试决策的主要依据是面试侧自身的数据：历史得分、掌握度、错题分布、距上次练习的天数。当学习方向与面试方向相交时（比如你在自习室备考 Java 并发，又要面 Java 并发），自习室的专注时长与完成率才作为辅助证据参与决策；不相交时（比如用自习室背政治、用 AI 面练后端），两条线互不干扰——信号按方向聚合，政治的时长永远算不到后端头上。而每一次决策的依据都落库、都可查看——系统必须能回答「你凭什么这么考我」。

可解释的面试决策是产品的灵魂，但它不要求你用自习室：只用其中一个功能，每个功能都完整成立。当你确实在为面试科目投入学习时，系统能看见并据此调整——这正是它区别于「普通番茄钟 + 随机出题面试工具」的地方。
...
**主张一 · 入口全部平级，地基统一。**
侧边栏上自习室 / 模拟面试 / 知识问答 / 计划日程是四个平级入口，谁也不是 VIP 功能，谁也不是谁的前置。实现上它们共用同一内核（身份、模型网关、知识库、学习信号、方向字典）。并列的是界面，共用的是地基；学习数据与面试数据只在「方向」这个维度上自然相遇，不强制绑定（见 §6）。
...
| 功能 | 说明 |
|---|---|
| 信号采集与归一 | 主输入 = 面试侧自身数据（逐题得分、题目难度、追问深度、距上次练习天数、覆盖题量）；增强输入 = 学习信号（专注时长带质量等级、任务完成率），按 direction_id 聚合，仅对存在学习记录的方向参与（无记录即空，见 §6.1） |
| 掌握度模型 | 每个「用户 × 方向」维护 `mastery`，指数遗忘 + 练习增益，见 §6 |
| 遗忘曲线驱动出题 | 掌握度衰减到阈值以下的方向，自动掺入复习题（复习题占比可配） |
| 薄弱方向反推考察重点 | 得分低 + 覆盖题量少的方向加权提升考察密度；若该方向同时存在自习室学习记录，「专注投入低」作为辅助证据一并呈现（只影响解释强度，不单独触发加权） |
...
这是整套系统的地基。自习室把它当科目维度（统计与热力图），面试把它当方向维度（题库组织与掌握度）——两侧**必须引用同一份方向字典**：主数据只维护一套，避免同物异名靠字符串猜；同时方向 ID 聚合让学习信号与面试信号天然按方向隔离，相交即增强、不相交即互不影响（见 [direction ADR](./specs/2026-09-25-direction-master-data-adr.md)）。
...
1. **主输入 = 面试侧自身数据**：逐题得分、题目难度、追问深度、覆盖题量、距上次练习天数（掌握度公式的全部输入，见 §6.2）。只用一侧功能（只面不学）时，决策层靠这层数据完整工作。
2. **增强输入 = 学习信号**（自习室专注时长、任务完成率）：**按 direction_id 聚合**，只有当学习方向与面试方向相交（该方向存在学习记录）时才参与；不存在记录时信号为空——这是正常形态而非降级。WEAK_DIRECTION 的「专注占比」只在该方向自身的学习记录内计算，**禁止跨方向比较**（背政治的时长对 Java 面试毫无意义）。
...
```json
{
  ...
  "reasons": [
    ...
    {"rule": "WEAK_DIRECTION", "target": "kb-408-os", "detail": "专项面试 3 题错 2 题；该方向存在学习记录，近 14 天专注占比 6%（辅助证据）"},
...
2. 某方向的投入全部来自 `SELF_REPORTED` 时，该方向的学习信号不参与调整（决策仍基于面试侧数据）；学习信号完全缺失（方向不相交）是正常形态而非降级，面板照常给出基于掌握度与错题的解释。
...
| 学习信号 | 自习室侧输入：专注时长（含质量等级）、任务完成率；按 direction_id 聚合，与面试侧信号（得分、间隔天数）在决策层 §6.1 分层使用 |
